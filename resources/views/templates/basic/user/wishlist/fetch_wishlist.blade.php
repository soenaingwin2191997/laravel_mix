    ...
    <li class="wishlist-card-list__item">
        <div class="wishlist-card-wrapper">
            ...
            <div class="wishlist-card-wrapper__icon">
                <button class="text--base confirmationBtn" data-action="{{ route('user.wishlist.remove', $wishlist->id) }}" data-question="@lang('Are you sure to remove this item?')" data-submit_text="btn btn--default btn-md" type="button"><i class="las la-times"></i></button>
            </div>
        </div>
    </li>
@empty
    <li class="text--danger text-center">{{ __($emptyMessage) }}</li>
@endforelse
